...
public class DisplayHistorique {
    ...
    private ObservableList<HistoriqueClub> allHistorique = FXCollections.observableArrayList();
    ...
    private void loadScene(String fxmlPath, Button button) {
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Failed to load stylesheet", "Details: " + e.getMessage());
        }
    }
    ...
    public void initialize() {
        ...
        modifierColumn.setCellFactory(param -> new TableCell<>() {
            private final Button btn = new Button("Modifier");
            {
                ...
                btn.setOnAction(event -> {
                    ...
                    showConfirmation("Confirmation", "Modifier cet historique ?", "Club: " + selectedHistorique.getNomClub(), () -> {
        ...
        deleteColumn.setCellFactory(param -> new TableCell<>() {
            private final Button btn = new Button("Delete");
            {
                ...
                btn.setOnAction(event -> {
                    HistoriqueClub selectedHistorique = getTableView().getItems().get(getIndex());
                    showConfirmation("Confirmation", "Supprimer cet historique ?", "Club: " + selectedHistorique.getNomClub(), () -> {
    ...
    public void loadHistorique() {
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur", "Failed to load historique: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void filterHistorique(String searchText) {
        if (searchText == null || searchText.trim().isEmpty()) {
            ...
            historiqueList.setAll(allHistorique.stream()
                    .filter(historique -> {
                        String nomClub = historique.getNomClub() != null ? historique.getNomClub().toLowerCase() : "";
    ...
    private void handleExport() {
        ...
        fileChooser.setInitialFileName("Historiques_" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("ddMMyyyy_HHmmss")) + ".pdf");
        ...
        try {
            ...
            float[] columnWidths = {100, 100, 150, 150, 150};
            ...
            for (HistoriqueClub historique : historiques) {
                String nomClub = historique.getNomClub() != null ? historique.getNomClub() : "N/A";
                String saisonFin = historique.getSaisonFin() != null ? dateFormat.format(historique.getSaisonFin()) : "N/A";

                table.addCell(new Cell().add(new Paragraph(String.valueOf(historique.getIdHistorique()))
                        .setFont(regularFont).setFontSize(8).setTextAlignment(TextAlignment.CENTER)));
                table.addCell(new Cell().add(new Paragraph(String.valueOf(historique.getIdJoueur()))
                        .setFont(regularFont).setFontSize(8).setTextAlignment(TextAlignment.CENTER)));
                table.addCell(new Cell().add(new Paragraph(nomClub)
                        .setFont(regularFont).setFontSize(8).setTextAlignment(TextAlignment.CENTER)));
                table.addCell(new Cell().add(new Paragraph(dateFormat.format(historique.getSaisonDebut()))
                        .setFont(regularFont).setFontSize(8).setTextAlignment(TextAlignment.CENTER)));
                table.addCell(new Cell().add(new Paragraph(saisonFin)
                        .setFont(regularFont).setFontSize(8).setTextAlignment(TextAlignment.CENTER)));
            }
            ...
            document.add(new Paragraph("NexusSport 2025 ©")
                    .setFont(regularFont).setFontSize(12).setTextAlignment(TextAlignment.RIGHT));

            InputStream logoStream = getClass().getResourceAsStream("/images/logo_horizantal.png");
            if (logoStream == null) {
    ...
    private void sortTableByColumn(String columnName) {
        ...
        switch (columnName) {
            ...
            case "ID Joueur":
                ...
                break;
            case "Nom Club":
                comparator = Comparator.comparing(HistoriqueClub::getNomClub, Comparator.nullsLast(String::compareTo));
                break;
            case "Saison Début":
    ...
    private void showConfirmation(String title, String header, String content, Runnable action) {
